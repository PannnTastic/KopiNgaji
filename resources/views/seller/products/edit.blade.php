                                ...
                                <div class="grid grid-cols-3 gap-3 mt-2">
                                    @foreach($product->images as $image)
                                        <div class="relative group">
                                            ...
                                            <form method="POST" action="{{ route('seller.product-images.destroy', $image) }}"
                                                class="absolute top-1 right-1" onsubmit="return confirm('Hapus gambar ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-700">
                                                    ✕
                                                </button>
                                            </form>
                                        </div>
